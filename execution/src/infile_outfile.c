...
static int	process_inputs(char **files, char **redirections, int i_infl)
{
	int	fd;
	int	i;
...
int	ft_do_redirections(char **files, char **redirections)
{
	...
	if (ft_len_redirections(redirections) == 1)
		return (to_single_redirection(files, redirections), 1);
	...
	if (process_outputs(files, redirections, i_outfl) == FAILURE)
		return (FAILURE);
	if (process_inputs(files, redirections, i_infl) == FAILURE)
		return (FAILURE);
	return (SUCCESS);
}
